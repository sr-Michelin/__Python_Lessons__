...
class Point:
    ...
    def __setattr__(self, key, value):
        ...
        if key == "WIDTH":
            print('\nТи мій атрибут не чіпай!')
        else:
            self.__dict__[key] = value
            # self.__x = value # для появи RecursionError

    def __getattr__(self, item):
        """При введені неіснуючого атрибута у екземпляр класу виводить його у консоль """
        print(f'\n__getattr__: {item}')

    def __delattr__(self, item):
        """При видаленні певного атрибута сповіщає у консоль"""
        print(f'\n__delattr__: {item}')
