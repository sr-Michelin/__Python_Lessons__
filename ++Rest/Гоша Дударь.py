...
"--------------------Нейронка-------Sigmoid--------------------------------------"
# Тихо сп#### и ушел, называется нашёл
# Ні##я не зрозуміло, але дуже, дуже цікаво
...
for iteration in range(1000):                                               # задаємо кількість ітерацій (1000) у циклі for
    output = 1 / (1 + exp(-(dot(training_set_inputs, synaptic_weights))))   # дофіга сигмоїда
    synaptic_weights += dot(training_set_inputs.T, (training_set_outputs - output) * output * (1 - output))# зміна ваг до певного моменту, поки  training_set_inputs-->training_set_outputs ('вчимо' нейронку)
